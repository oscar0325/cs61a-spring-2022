...
def more_boar(player_score, opponent_score):
    ...
    a = score_to_str(player_score)
    b = score_to_str(opponent_score)
    for i in range(0, 2):
        if a[i] >= b[i]:
            return False
    return True
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    '''
    def one_round(play_score, opponent_score, strategy, say, who):
        play_score += take_turn(strategy(play_score, opponent_score), opponent_score, dice, goal)
        if who == 1:
            say = say(opponent_score, play_score)
        else:
            say = say(play_score, opponent_score)
        while more_boar(play_score, opponent_score):
            play_score += take_turn(strategy(play_score, opponent_score), opponent_score, dice, goal)
            if who == 1:
                say = say(opponent_score, play_score)
            else:
                say = say(play_score, opponent_score)
        return play_score, say

    while score0 < goal and score1 < goal:
            score0, say = one_round(score0, score1, strategy0, say, who)
            if score0 >= goal:
                break
            who = next_player(who)
            score1, say = one_round(score1, score0, strategy1, say, who)
            who = next_player(who)
    '''
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for i in range(1, 11):
        average_score = averaged_dice(i, dice)
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    # score_after_more_boar = score + piggy_points(opponent_score)
    # if more_boar(score_after_more_boar, opponent_score):
    #     return 0
    # else:
    #     piggypoints_strategy(score, opponent_score, cutoff, num_rolls)
    score_after = score + piggy_points(opponent_score)
    if more_boar(score_after, opponent_score):
        return 0
    else:
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    if score < 50:
        return more_boar_strategy(score, opponent_score, cutoff=10, num_rolls=10)
    else:
        return more_boar_strategy(score, opponent_score, cutoff=10, num_rolls=6)
