...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    paragraphs_sel = [ps for ps in paragraphs if select(ps)]
    return paragraphs_sel[k] if k < len(paragraphs_sel) else ''
...
def accuracy(typed, reference):
    ...
    if typed_length == reference_length == 0:
        return 100.0
    ...
    if typed_length != 0 and reference_length == 0:
        return 0.0
    count = 0
    for a, b in zip(typed_words, reference_words):
        if a == b:
            count += 1
    return  count / typed_length * 100
...
def sphinx_switches(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def equal(start, goal, count):
        if count > limit:
            return limit + 1
        if not start and not goal: #如果两者为空，返回count
            return count
        if not start or not goal: #如果一个空，另一个不空继续计数
            return equal(start[1:], goal[1:], count + 1)
        else: #如果两者都不为空
            if start[0] == goal[0]: #如果两个元素相等，不增加计数
                return equal(start[1:], goal[1:], count)
            else: #否则加1
                return equal(start[1:], goal[1:], count +  1)
    return equal(start, goal, 0)
    ...
    # else:
    #     length = len(start)
    # count = abs(len(start) - len(goal)) #对修改的字符数进行计数
...
def time_per_word(times_per_player, words):
    ...
    for player_time in times_per_player:
        ...
        for i in range(len(words)):
            time_per_word.append(player_time[i + 1] - player_time[i])
        p.append(time_per_word)
    return game(words, p)
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest_time_word = [ [] for player_num in player_indices ]
    for i in word_indices:
        fastest_player = min([(player_num, time(game, player_num, i)) for player_num in player_indices ],
                                key=lambda type_time: type_time[1])
        fastest_time_word[fastest_player[0]].append(all_words(game)[i])
    return fastest_time_word
