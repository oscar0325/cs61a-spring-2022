...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def record(index, ppn, direct):
        if index == n:
            return ppn
        if index % 8 == 0 or num_eights(index) > 0:
            return record(index+1, ppn-direct, -direct)
        else:
            return record(index+1, ppn+direct, direct)
...
def count_coins(change):
    ...
    def conut_helper(changes, coin):
        if changes == 0:
            ...
            return conut_helper(changes, get_next_coin(coin)) + conut_helper(changes - coin, coin)
    return conut_helper(change, 1)
