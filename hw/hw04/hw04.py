...
def balanced(m):
    ...
    return total_weight(end(left(m))) * length(left(m)) == total_weight(end(right(m))) * length(right(m)) \
            and balanced(left(m)) and balanced(right(m))
...
def mul_interval(x, y):
    ...
    value in y."""
    p1 = lower_bound(interval(x, y)) * lower_bound(interval(x, y))
    p2 = lower_bound(interval(x, y)) * upper_bound(interval(x, y))
    p3 = upper_bound(interval(x, y)) * lower_bound(interval(x, y))
    p4 = lower_bound(interval(x, y)) * lower_bound(interval(x, y))
